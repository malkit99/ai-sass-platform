...
const schema = toTypedSchema(
  yup.object({
    ...
    media_url: yup.string().required('Document URL is required').url('Enter a valid URL'),
...
const submit = handleSubmit(async (values) => {
  saving.value = true
  try {
    const payload = { ...values, type: 'text_document' }
    if (props.editing) {
...
<template>
  <TemplateFormShell badge="DOCUMENT" :editing="!!editing" :saving="saving" @back="$emit('back')" @save="submit">
    <v-card class="pa-4 mb-4">
      <div class="text-caption text-medium-emphasis mb-1">TEMPLATE NAME *</div>
      <v-text-field
        v-model="name" v-bind="nameAttrs" placeholder="Enter a descriptive name (e.g. Price List PDF)"
    ...
    <v-card class="pa-4 mb-4">
      ...
      <div class="text-caption text-medium-emphasis mb-1">DOCUMENT URL *</div>
    ...
    <MediaPickerDialog v-model="showPicker" type="document" @selected="mediaUrl = $event" />
    ...
    <template #preview>
      <TemplatePhonePreview
        icon="mdi-file-document-outline" media-type="document" :media-url="mediaUrl" :body-text="body" :footer-text="footer"
      />
    </template>
  </TemplateFormShell>
</template>
