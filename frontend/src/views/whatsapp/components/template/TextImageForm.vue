...
const submit = handleSubmit(async (values) => {
  saving.value = true
  try {
    const payload = { ...values, type: 'text_image' }
    if (props.editing) {
...
<template>
  <TemplateFormShell badge="IMAGE" :editing="!!editing" :saving="saving" @back="$emit('back')" @save="submit">
    <v-card class="pa-4 mb-4">
      <div class="text-caption text-medium-emphasis mb-1">TEMPLATE NAME *</div>
      <v-text-field
        v-model="name" v-bind="nameAttrs" placeholder="Enter a descriptive name (e.g. Product Photo)"
    ...
    <v-card class="pa-4 mb-4">
      ...
      <div class="text-caption text-medium-emphasis mb-1">IMAGE URL *</div>
    ...
    <template #preview>
      <TemplatePhonePreview
        icon="mdi-image-outline" media-type="image" :media-url="mediaUrl" :body-text="body" :footer-text="footer"
      />
    </template>
  </TemplateFormShell>
</template>
